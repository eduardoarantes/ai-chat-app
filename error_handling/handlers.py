...
class ErrorContextCapture:
    ...
    async def capture_exception_context(
        self, exception: Exception, session_id: Optional[str] = None, additional_data: Optional[Dict[str, Any]] = None
    ) -> ErrorContext:
        """
...
class ErrorMessageTranslator:
    ...
    def __init__(self):
        ...
        self._translation_rules = {
            ...
            ConfigurationError: {
                ...
                "suggested_actions": ["Contact technical support", "Try again later", "Report this error with the error ID"],
    ...
    def translate_error(
        self, exception: Exception, context: ErrorContext, fallback_message: Optional[str] = None
    ) -> ErrorResult:
        """
...
class ErrorHandler:
    ...
    async def handle_error(
        ...
        try:
            ...
            if request:
                context = await self.context_capture.capture_request_context(request, session_id, additional_context)
            else:
                context = await self.context_capture.capture_exception_context(exception, session_id, additional_context)
...
class ErrorHandlingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        try:
            ...
            # Handle the error
            error_result = await self.error_handler.handle_error(e, request, session_id)
...
class SSEErrorStreamer:
    ...
    async def stream_error(
        self, exception: Exception, session_id: Optional[str] = None, request: Optional[Request] = None
        ...
        try:
            # Handle the error
            error_result = await self.error_handler.handle_error(exception, request, session_id)
...
class ErrorResponseHandler:
    ...
    async def handle_json_error(
        self, exception: Exception, request: Optional[Request] = None, session_id: Optional[str] = None
    ) -> JSONResponse:
    ...
    def _get_http_status_code(self, error_result: ErrorResult) -> int:
        ...
        return category_status_map.get(error_result.category, severity_status_map.get(error_result.severity, 500))
