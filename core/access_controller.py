...
from models.session import EnhancedSession, SessionDocument, DocumentAccessLevel
...
class DocumentAccessController:
    ...
    def update_access_policy(self, session_id: str, policy_updates: Dict[str, Any]) -> bool:
        ...
        allowed_fields = {
            'allow_sharing_requests', 'allow_receiving_shares', 'max_shared_documents',
            'max_received_documents', 'trusted_sessions', 'blocked_sessions',
            'auto_approve_from_trusted', 'sharing_request_ttl_hours'
        }
    ...
    def create_sharing_request(
        ...
        if (target_policy.auto_approve_from_trusted and 
            source_session_id in target_policy.trusted_sessions):
    ...
    def approve_sharing_request(self, request_id: str, approving_session_id: str) -> Tuple[bool, Optional[str]]:
        ...
        self._add_to_audit_log("request_approved", {
    ...
    def reject_sharing_request(
        ...
        # Log to audit history
        self._add_to_audit_log("request_rejected", {
            "request_id": request_id,
            "source_session": request.source_session_id,
            "target_session": request.target_session_id,
            "document_hash": request.document_hash,
    ...
    def cleanup_expired_requests(self) -> int:
        ...
        for request_id in expired_requests:
            ...
            if request:
                ...
                # Log cleanup
                self._add_to_audit_log("request_expired", {
                    "request_id": request_id,
                    "source_session": request.source_session_id,
                    "target_session": request.target_session_id,
                    "document_hash": request.document_hash,
    ...
    def revoke_document_sharing(self, source_session_id: str, document_hash: str) -> int:
        ...
        # Log revocation
        self._add_to_audit_log("sharing_revoked", {
    ...
    def get_access_statistics(self) -> Dict[str, Any]:
        ...
        expired_count = sum(1 for req in self._pending_requests.values() 
                          if req.expires_at and now > req.expires_at and req.approved is None)
