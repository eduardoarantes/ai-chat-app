...
class SessionDocumentManager:
    ...
    def add_document_to_session(
        self, session_id: str, document_metadata: DocumentMetadata, chunked_document: Optional[ChunkedDocument] = None
        ...
        session_document = SessionDocument(
            document_metadata=document_metadata, chunks=chunked_document, access_level=DocumentAccessLevel.PRIVATE, owned=True
        )
    ...
    def cleanup_failed_documents(self) -> int:
        ...
        if total_cleaned > 0 or orphaned_cleaned > 0:
            self._logger.info(
                f"Cleaned up {total_cleaned} failed documents from sessions, "
    ...
    def get_manager_statistics(self) -> Dict[str, Any]:
        ...
        return {"document_store": store_stats, "sessions": session_stats, "configuration": config_stats}
    ...
    def _validate_session_limits(self, session: EnhancedSession, new_document_size: int) -> Optional[str]:
        ...
        if current_size + new_document_size > self._max_total_size_per_session:
            return (
                f"Adding document would exceed size limit "
