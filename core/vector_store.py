...
class VectorStore:
    ...
    async def _get_or_create_collection(self, session_id: str, metadata: Optional[Dict[str, Any]] = None) -> Collection:
        ...
        try:
            ...
            collection = self.client.get_or_create_collection(name=collection_name, metadata=collection_metadata)
    ...
    async def list_collections(self) -> List[Dict[str, Any]]:
        ...
        try:
            collections = self.client.list_collections()
            return [{"name": col.name, "id": col.id, "metadata": col.metadata or {}} for col in collections]
        except Exception as e:
    ...
    async def store_embedding(self, embedding: ChunkEmbedding) -> bool:
        ...
        if len(embedding.embedding_vector) != self.expected_dimension:
            raise VectorStoreError(
                f"Embedding dimension mismatch: expected {self.expected_dimension}, " f"got {len(embedding.embedding_vector)}"
            )
        ...
        try:
            ...
            # Store in ChromaDB
            collection.add(ids=[embedding.chunk_id], embeddings=[embedding.embedding_vector], metadatas=[metadata])
    ...
    async def similarity_search(
        ...
        if len(query_vector) != self.expected_dimension:
            raise VectorStoreError(
                f"Query vector dimension mismatch: expected {self.expected_dimension}, " f"got {len(query_vector)}"
            )
    ...
    async def update_embedding(
        self, chunk_id: str, session_id: str, new_vector: List[float], metadata_updates: Optional[Dict[str, Any]] = None
    ) -> bool:
        ...
        try:
            ...
            # Prepare update data
            update_data = {"ids": [chunk_id], "embeddings": [new_vector]}
    ...
    async def get_embeddings_by_document(self, document_id: str, session_id: str) -> List[ChunkEmbedding]:
        ...
        try:
            ...
            for collection_info in collections:
                try:
                    ...
                    health["collections"][collection_info.name] = {"count": count, "metadata": collection_info.metadata or {}}
                except Exception as e:
            ...
            health["performance"]["last_backup"] = self._last_backup.isoformat() if self._last_backup else None
