...
from models.search import ChunkEmbedding, EmbeddingMetadata, SimilarityMetric
...
class EmbeddingService:
    ...
    async def _warm_up_model(self) -> None:
        ...
        try:
            await asyncio.get_event_loop().run_in_executor(None, self.model.encode, warm_up_texts)
    ...
    async def generate_embedding(self, chunk: DocumentChunk) -> np.ndarray:
        ...
        try:
            ...
            embedding_matrix = await asyncio.get_event_loop().run_in_executor(None, self.model.encode, [chunk.content], 1)
            ...
            if embedding_matrix.shape[1] != self.embedding_dimension:
                raise EmbeddingError(
                    f"Unexpected embedding dimension: expected {self.embedding_dimension}, " f"got {embedding_matrix.shape[1]}"
                )
