...
from error_handling.handlers import ErrorContextCapture, ErrorHandler, ErrorMessageTranslator

# Import the error handling components we'll implement
from models.errors import (  # Existing exceptions from TASK-001; New exceptions; Error handling infrastructure
    ApplicationError,
    ConfigurationError,
...
class TestEnhancedExceptionHierarchy:
    ...
    def test_application_error_defaults(self):
        ...
        error = ApplicationError(message="Test error", error_code="TEST_001", severity=ErrorSeverity.MEDIUM)
        ...
        with patch("main.uuid.uuid4", return_value=Mock(hex="test-error-id")):
            with patch("main.datetime.datetime") as mock_datetime:
                ...
                context = await capture.capture_request_context(
                    request=mock_request, session_id=session_id, additional_data={"custom": "data"}
                )
        ...
        try:
            ...
            with patch("main.uuid.uuid4", return_value=Mock(hex="test-error-id")):
                with patch("main.datetime.datetime") as mock_datetime:
                    ...
                    context = await capture.capture_exception_context(exception=e, session_id="test-session")
...
class TestErrorMessageTranslator:
    ...
    def test_translate_session_error(self):
        ...
        error = SessionError(message="Session not found in storage", error_code="SESSION_001", severity=ErrorSeverity.MEDIUM)
        result = translator.translate_error(error)
    ...
    def test_translate_llm_error(self):
        ...
        error = LLMError(message="Google API rate limit exceeded: 429", error_code="LLM_001", severity=ErrorSeverity.HIGH)
        result = translator.translate_error(error)
    ...
    def test_translate_network_error(self):
        ...
        error = NetworkError(
            message="Connection timed out after 30 seconds", error_code="NETWORK_001", severity=ErrorSeverity.HIGH
        )
        result = translator.translate_error(error)
...
class TestErrorHandler:
    ...
    async def test_process_error_with_request_context(self, error_handler, mock_request):
        ...
        error = SessionError(message="Session expired", error_code="SESSION_001", severity=ErrorSeverity.MEDIUM)
        ...
        with patch.object(error_handler.context_capture, "capture_request_context") as mock_capture:
            ...
            result = await error_handler.process_error(error=error, request=mock_request, session_id="test-session")
    ...
    async def test_process_error_without_request_context(self, error_handler):
        ...
        error = LLMError(message="API error", error_code="LLM_001", severity=ErrorSeverity.HIGH)
