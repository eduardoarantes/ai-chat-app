...
from models.session import (
    DocumentMetadata, SessionDocument, SessionAccessControls, EnhancedSession,
    DocumentProcessingStatus, DocumentAccessLevel
)
...
class TestSessionDocument:
    ...
    def test_session_document_can_share(self):
        ...
        shared_doc = SessionDocument(
            document_metadata=doc_metadata,
            access_level=DocumentAccessLevel.SHARED,
            owned=True
        )
        assert shared_doc.can_share()
        
        # Not owned document - cannot share
        not_owned_doc = SessionDocument(
            document_metadata=doc_metadata,
            access_level=DocumentAccessLevel.SHARED,
            owned=False
        )
    ...
    def test_session_document_accessibility(self):
        ...
        shared_from_doc = SessionDocument(
            document_metadata=doc_metadata,
            owned=False,
            shared_from="source-session"
        )
