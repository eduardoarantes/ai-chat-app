...
from models.validation import ValidationResult, ValidationStatus, FileValidationConfig
...
class TestFileValidator:
    ...
    async def test_validate_file_mime_type_restriction(self, mock_text_file):
        ...
        config = FileValidationConfig(
            allowed_mime_types={'image/png', 'image/jpeg'}  # Only images allowed
        )
        validator = FileValidator(config)
    ...
    async def test_validate_file_security_failure(self, mock_text_file):
        ...
        config = FileValidationConfig(
            security_threshold=0.9,  # Very high threshold
            enable_security_scanning=True
        )
        validator = FileValidator(config)
