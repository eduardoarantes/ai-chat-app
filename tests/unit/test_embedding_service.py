...
from core.embedding_service import EmbeddingService, EmbeddingMetadata, EmbeddingError
...
class TestEmbeddingService:
    ...
    class TestConfiguration:
        ...
        async def test_different_similarity_metrics(self, embedding_service):
            ...
            cosine_sim = embedding_service.compute_similarity(
                embedding1, embedding2, metric="cosine"
            )
            assert 0.0 <= cosine_sim <= 1.0
            
            # Test euclidean similarity  
            euclidean_sim = embedding_service.compute_similarity(
                embedding1, embedding2, metric="euclidean"
            )
            assert isinstance(euclidean_sim, float)
            
            # Test dot product
            dot_sim = embedding_service.compute_similarity(
                embedding1, embedding2, metric="dot_product"
            )
