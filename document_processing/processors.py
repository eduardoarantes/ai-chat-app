...
class TemporaryFileManager:
    ...
    def create_temp_file(self, suffix: str = None, prefix: str = "langchain_") -> Tuple[str, str]:
        ...
        temp_file = tempfile.NamedTemporaryFile(delete=False, suffix=suffix, prefix=prefix, mode="wb")
...
class PDFProcessor:
    ...
    async def process_pdf(self, file_content: bytes, filename: str, session_id: Optional[str] = None) -> ProcessedDocument:
        """
        ...
        try:
            ...
            temp_file_id, temp_file_path = self.temp_file_manager.create_temp_file(
                suffix=".pdf", prefix=f"pdf_process_{filename}_"
            )
            ...
            error_result = await self.error_handler.handle_error(
                exception=e, session_id=session_id, additional_context={"filename": filename, "processing_type": "pdf"}
            )
    ...
    def _extract_pdf_metadata(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "page_dimensions": [
                    {"page": i + 1, "width": page.rect.width, "height": page.rect.height} for i, page in enumerate(doc)
                ],
            }
        except Exception as e:
            logging.error(f"Error extracting PDF metadata: {e}")
            return {"title": "", "author": "", "page_count": 0, "pdf_version": "unknown", "error": str(e)}
        finally:
            if "doc" in locals():
                doc.close()
    ...
    def _process_pdf_pages(self, pdf_path: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for page_num in range(doc.page_count):
                ...
                pages.append({"page_number": page_num + 1, "content": page_content, "metadata": page_metadata})
...
class TextProcessor:
    ...
    async def process_text(self, file_content: bytes, filename: str, session_id: Optional[str] = None) -> ProcessedDocument:
        ...
        try:
            ...
            # Create temporary file
            temp_file_id, temp_file_path = self.temp_file_manager.create_temp_file(
                suffix=file_ext, prefix=f"text_process_{filename}_"
            )
            ...
            # Use error handler for structured error processing
            error_result = await self.error_handler.handle_error(
                exception=e, session_id=session_id, additional_context={"filename": filename, "processing_type": "text"}
            )
    ...
    def _analyze_csv_structure(self, content: str) -> Dict[str, Any]:
        ...
        delimiter = (
            ...
            else (";" if semicolon_count >= tab_count else "\t")
        )
    ...
    def _analyze_markdown_structure(self, content: str) -> Dict[str, Any]:
        ...
        return {
            "markdown_headings": {"h1": h1_count, "h2": h2_count, "h3": h3_count},
...
class UnstructuredProcessor:
    ...
    async def process_document(
        ...
        try:
            ...
            # Create temporary file
            temp_file_id, temp_file_path = self.temp_file_manager.create_temp_file(
                suffix=file_ext, prefix=f"unstructured_process_{filename}_"
            )
...
class DocumentProcessorFactory:
    ...
    async def process_document(
        self, file_content: bytes, filename: str, mime_type: str, session_id: Optional[str] = None
        ...
        try:
            ...
            error_result = await self.error_handler.handle_error(
                exception=e,
                session_id=session_id,
                additional_context={"filename": filename, "mime_type": mime_type, "processing_type": "document_loader"},
            )
...
class ChunkOptimizer:
    ...
    def optimize_chunk_size(self, text: str, metadata: Dict[str, Any]) -> int:
        ...
        adjusted_size = max(self.config.min_chunk_size, min(adjusted_size, self.config.max_chunk_size))
...
class ChunkProcessor:
    ...
    async def process_document(
        self, document: ProcessedDocument, session_id: Optional[str] = None
        ...
        try:
            ...
            chunks = strategy.chunk_text(
                content, {"document_id": document.original_filename, "mime_type": mime_type, **document.metadata}
            )
            ...
            chunked_document = ChunkedDocument(
                original_document=document, chunks=chunks, chunk_index=chunk_index, chunking_metadata=chunking_metadata
            )
            ...
            try:
                await self.error_handler.handle_error(
                    exception=e,
                    session_id=session_id,
                    additional_context={"filename": document.original_filename, "processing_type": "chunking"},
                )
