...
from models.errors import (
    ContentValidationError, FileSizeError, MimeTypeError, 
    SecurityError, LLMError, NetworkError, ErrorSeverity
)
from models.session import DocumentMetadata, SessionDocument, DocumentProcessingStatus, DocumentAccessLevel
from error_handling.handlers import SSEErrorStreamer, ErrorHandler
    ...
    if file:
        ...
        if validation_result.status == ValidationStatus.ERROR:
            ...
            return StreamingResponse(
                sse_streamer.stream_error(error, session_id, request),
            ...
            error = SecurityError(f"File flagged as potentially suspicious (security score: {validation_result.security_score:.2f})")
            return StreamingResponse(
                sse_streamer.stream_error(error, session_id, request),
                media_type="text/event-stream"
            )
        ...
        logging.info(f"File validation passed: {file.filename}, hash: {validation_result.file_hash}, "
...
{json.dumps(processed_document.structure, indent=2)}"""
            ...
            logging.info(f"Enhanced processing completed for {file.filename}: "
            ...
            if document_metadata:
                ...
                try:
                    success, error = session_document_manager.add_document_to_session(
                        session_id, document_metadata, None
                    )
                    if success:
            ...
            if mime_type.startswith('image/'):
                ...
                try:
                    ...
                    if mime_type.startswith('text/') or mime_type in ['application/json', 'application/xml']:
                        ...
                        user_message_content.append({
                            "type": "text",
                            "text": f"File '{file.filename}' content ({mime_type}):\n```\n{file_text}\n```"
                        })
                    else:
        ...
        logging.info(f"Completed file processing for: {file.filename}, "
...
async def get_session_documents(session_id: str, include_chunks: bool = False):
    ...
    if not session_document_manager:
        return JSONResponse(
            content={"error": "Document persistence not enabled"}, 
            status_code=501
        )
...
async def get_document_details(document_id: str, session_id: Optional[str] = None):
    ...
    try:
        ...
        if not session_document:
            return JSONResponse(
                content={"error": "Document not found in any session"}, 
                status_code=404
            )
...
async def share_document(
    ...
    if not session_document_manager or not access_controller:
        return JSONResponse(
            content={"error": "Document sharing not enabled"}, 
            status_code=501
        )
    ...
    try:
        ...
        if not source_document:
            return JSONResponse(
                content={"error": "Document not found in source session"}, 
                status_code=404
            )
        
        # Check if sharing is allowed
        can_share, reason = access_controller.can_share_document(
            source_session_id, target_session_id, source_document
        )
        if not can_share:
            return JSONResponse(
                content={"error": f"Sharing not allowed: {reason}"}, 
                status_code=403
            )
        ...
        if success:
            ...
            return JSONResponse(
                content={"error": f"Sharing failed: {error}"}, 
                status_code=400
            )
...
async def remove_document(document_id: str, session_id: str = Query(...)):
    ...
    try:
        ...
        if not session_document:
            return JSONResponse(
                content={"error": "Document not found in session"}, 
                status_code=404
            )
        ...
        if removed:
            ...
            return JSONResponse(
                content={"error": "Failed to remove document from session"}, 
                status_code=500
            )
...
async def get_cleanup_statistics():
    ...
    if not cleanup_manager:
        return JSONResponse(
            content={"error": "Cleanup manager not available"}, 
            status_code=501
        )
