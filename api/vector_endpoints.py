...
from fastapi import HTTPException, Query, Path, BackgroundTasks
from fastapi.responses import JSONResponse
from sse_starlette import EventSourceResponse

from core.embedding_service import EmbeddingService, EmbeddingError
from core.vector_store import VectorStore, VectorStoreError
from core.semantic_search import SemanticSearchService, SemanticSearchError
from models.search import (
    SemanticSearchRequest, SemanticSearchResponse, 
    SimilarDocumentsRequest, SimilarDocumentsResponse,
    EmbeddingGenerationRequest, EmbeddingGenerationResponse,
    EmbeddingStatusResponse, VectorDatabaseStats,
    SearchType, SimilarityMetric, EmbeddingStatus
)
...
def _check_services_initialized() -> None:
    ...
    if not embedding_service or not vector_store or not semantic_search_service:
        raise HTTPException(
            status_code=500,
            detail="Vector services not initialized"
        )
...
async def semantic_search(request: SemanticSearchRequest) -> SemanticSearchResponse:
    ...
    try:
        ...
        logger.info(
            f"Semantic search completed: {len(response.results)} results in {response.search_time_ms:.1f}ms"
        )
...
async def generate_embeddings(
    ...
    try:
        ...
        if not request.force_regenerate:
            ...
            existing_embeddings = await vector_store.get_embeddings_by_document(
                request.document_id, request.session_id
            )
...
async def get_embedding_status(
    ...
    try:
        ...
        progress_percentage = (
            (len(embeddings) / total_chunks) * 100.0
            if total_chunks > 0 else 100.0
        )
